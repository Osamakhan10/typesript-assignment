...
//u/sed. If it has, print a message that the person will need to enter a new username. If a 
...
const current_users = ['khalid', 'Alice', 'fawad', 'Charlie', 'David'];
const new_users = ['John', 'Kate', 'George', 'Alice', 'Lucy'];
for (const newUsername of new_users) {
    const usernameTaken = current_users.some(existingUsername => existingUsername.toLowerCase() === newUsername.toLowerCase());
    if (usernameTaken) {
